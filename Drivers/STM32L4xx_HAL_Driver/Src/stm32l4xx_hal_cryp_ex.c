...
HAL_StatusTypeDef HAL_CRYPEx_AES_Auth_IT(CRYP_HandleTypeDef *hcryp, uint8_t *pInputData, uint64_t Size, uint8_t *pOutputData)
{
    ...
    mask[0][0] = 0xFF000000U;
    mask[0][1] = 0xFFFF0000U;
    mask[0][2] = 0xFFFFFF00U; /* 32-bit data */
    mask[1][0] = 0x0000FF00U;
    mask[1][1] = 0x0000FFFFU;
    mask[1][2] = 0xFF00FFFFU; /* 16-bit data */
    mask[2][0] = 0x000000FFU;
    mask[2][1] = 0x0000FFFFU;
    mask[2][2] = 0x00FFFFFFU; /* 8-bit data  */
    mask[3][0] = 0x000000FFU;
    mask[3][1] = 0x0000FFFFU;
    mask[3][2] = 0x00FFFFFFU; /* Bit data    */
        ...
        /*============================================*/
           ...
           bytes in last block of payload */
                if (READ_BIT(hcryp->Instance->CR, AES_CR_GCMPH) == CRYP_PAYLOAD_PHASE) {
                    ...
                    if (((cr_temp & (AES_CR_CHMOD | AES_CR_MODE)) == (CRYP_CHAINMODE_AES_GCM_GMAC | CRYP_ALGOMODE_ENCRYPT)) || ((cr_temp & (AES_CR_CHMOD | AES_CR_MODE)) == (CRYP_CHAINMODE_AES_CCM | CRYP_ALGOMODE_DECRYPT))) {
